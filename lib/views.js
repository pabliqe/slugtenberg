...
function handleDatasheets(filepath, globals) {
	...
	let options = {
		name: parser.make(filepath)
	}

	// Search for datasheet configuration files
	let optionsPath = path.dirname(filepath) + '/' + options.name + '.yml';
	if(fs.existsSync(optionsPath)) {
		extend(options, parser.read(optionsPath));
	
		log('Data config found:', options);
	}
	...
	if(options.use_layout) {
		loaded.forEach((v,i,a) => {
			...
			src('src/layouts/' + options.use_layout + '+(' + config.typeTemplate + ')')
				.pipe(rename((f) => log('▶︎▶︎▶︎ Building view (@data): ' + options.name + '/' + v.slug)))
...
function handleDataSlugs(filepath, globals) {
	...
	log('Data loaded: ' + Object.keys(loaded).length)

	// Save to global object
	globals[parser.safe(filepath)] = loaded;
}
...
module.exports = (cb) => {
	...
	fs.readdirSync(dir)
		...
		.forEach((v,i,a) => {
			console.log(v, fs.lstatSync(dir + '/' + v).isDirectory());
