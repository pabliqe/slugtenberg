...
function handleDataSheets(filepath, globals) {
	logb('Loading data from $1', [filepath])
	...
	loga('Configuration for ' + options.name, options);
	...
	loga('Data loaded from ' + options.name, loaded.length + ' ' + typeof loaded + 's')
	...
	if(options.use_layout) {
		loaded.forEach((v,i,a) => {
			...
			src(`${config.rootPath}/${config.src}/layouts/${options.use_layout}+(${config.typeTemplate})`)
				.pipe(logf('Building view@data', options.name + '/' + v.slug))
...
function handleDataFiles(filepath, globals) {
	...
	loga('Data loaded for ' + s, Object.keys(loaded).length + ' ' + typeof loaded + 's')
...
module.exports = function buildViews (cb) {

	// Append global data files
	let dir = `${config.rootPath}/${config.src}/data`
	...
	return src([
		`${config.rootPath}/${config.src}/slugs/**/*+(${config.typeTemplate})`,  // Ext force to read files only
